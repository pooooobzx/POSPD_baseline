...
class Transformer(nn.Module):
    def __init__(self,src_vocab_size, tgt_vocab_size, d_model, encoder_layer, decoder_layer, d_k, d_v, n_heads,d_ff):
        super(Transformer, self).__init__()
        self.encoder = Encoder(src_vocab_size, d_model, encoder_layer, d_k, d_v, n_heads, d_ff).to(device)
        self.decoder = Decoder(tgt_vocab_size, d_model, decoder_layer, d_k, d_v, n_heads, d_ff).to(device)
        self.projection = nn.Linear(d_model, tgt_vocab_size, bias=False).to(device)
        self.projection2 = nn.Linear(d_model, src_vocab_size, bias=False).to(device)
    def forward(self, enc_inputs, dec_inputs):
        ...
        dec_logits2 = self.projection2(dec_outputs)
        return dec_logits.view(-1, dec_logits.size(-1)), enc_self_attns, dec_self_attns, dec_enc_attns,dec_logits2.view(-1, dec_logits2.size(-1))
